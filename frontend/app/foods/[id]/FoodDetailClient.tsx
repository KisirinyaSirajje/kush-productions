...
export default function FoodDetailClient({ food, relatedFoods }: FoodDetailClientProps) {
  ...
  const favorited = isFavorited('food', food.id);
  const favorite = favorites.find((f) => f.type === 'food' && f.foodId === food.id);
    ...
    <>
      ...
      <section className="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto space-y-8">
          <RatingInput type="food" itemId={food.id} onRatingSubmitted={() => window.location.reload()} />
          <CommentSection type="food" itemId={food.id} />
        </div>
      </section>
